...
| recipe_name | string | 饮品名称 |
| cup_size | string | 杯型：小/中/大 |
| created_at | string | 制作完成时间（ISO 8601 UTC） |
...
| 级别 | 名称 | 说明 | 设备行为 |
|------|------|------|---------|
| warning | 警告 | 不中断运行 | 屏幕提示 + 告警记录，5分钟合并上报 |
| error | 错误 | 影响运行 | 停止制作，等待人工处理；危险项立即断电 |

**检测项目：**

| 检测项 | 检测方式 | 级别 | 周期 |
|--------|---------|------|------|
| 加热器异常 | 温度传感器 | error | 1秒 |
| 水泵异常 | 流量传感器 | error | 1秒 |
| 电机异常 | 电流检测 | error | 500ms |
| 漏水检测 | 漏水传感器 | error | 500ms |
| 通信模块异常 | 心跳超时 | warning | 5秒 |
| 传感器离线 | 通信超时 | warning | 5秒 |
| 原料不足 | 传感器阈值 | warning | 10秒 |

**需求描述：**

1. error 级故障即时告警上报，L1/L2级合并周期上报（5分钟）
2. error 级故障中危险项设备自动进入安全锁定状态，需人工现场复位
3. 告警信息包含：设备ID、故障码、故障描述、发生时间
4. 故障恢复后发送恢复通知

**验收标准：**
- error 级故障检测到保护动作 ≤ 200ms
- 告警上报延迟（error） ≤ 3秒
...
| 故障码 | 名称 | 级别 | 说明 |
|--------|------|------|------|
| E001 | HEATER_OVERTEMP | error | 加热器超温 |
| E002 | HEATER_NOT_HEATING | error | 加热器不加热 |
| E003 | PUMP_FAILURE | error | 水泵故障 |
| E004 | MOTOR_STALL | error | 电机堵转 |
| E005 | WATER_LEAK | error | 漏水检测 |
| W001 | SENSOR_OFFLINE | warning | 传感器离线 |
| W002 | COMM_FAIL | warning | 通信模块异常 |
| W003 | MATERIAL_LOW | warning | 原料不足 |
