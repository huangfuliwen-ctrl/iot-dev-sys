...
│                                                  │
│  下行指令:                                        │
│  publish → {tenant}/iot/{product}/{device}/cmd   │
└──────────────────────────────────────────────────┘
```
...
```
设备上行Topic: {tenant_id}/iot/{product_id}/{device_id}/event/post
                                  │
                    MQTT订阅: +/iot/+/+/event/post
                                  │
                    收到消息后解析topic各段:
                      tenant_id  = "tenant_1"
...
publish → tenant_1/iot/coffee_v1/device_001/ota/notify {payload}
```
...
```
# 上行消费 (通配符订阅，接收所有设备消息)
订阅: +/iot/+/+/event/post       ← 所有设备事件
订阅: +/iot/+/+/property/post    ← 所有设备属性
订阅: +/iot/+/+/heartbeat        ← 所有设备心跳
订阅: +/iot/+/+/ota/progress     ← 所有OTA进度

# 下行发送 (精确topic，定向到具体设备)
发布: {tenant}/iot/{product}/{device}/property/set
发布: {tenant}/iot/{product}/{device}/ota/notify
发布: {tenant}/iot/{product}/{device}/command/{cmd}
```
...
2. 向云平台发起激活请求，上传 `uid` + `model_key`
3. 云平台根据 `model_key` 查找设备型号，自动解析：设备类型（COFFEE_MACHINE等）、产品ID、基础固件版本
4. 云平台分配 `device_id` 并生成 `activation_token`（256位随机hex）
5. 返回：device_id, model_code, device_type, product_id, firmware_version, token, broker_uri
6. 设备将凭证安全存储
7. 激活成功进入正常运行模式
8. 激活失败：首次间隔30秒重试，之后指数退避，最大间隔5分钟
...
| 订阅 Topic | QoS | 用途 |
|-----------|-----|------|
| `{tenant}/iot/{product}/{device}/property/set` | 1 | 接收云端配置下发 |
| `{tenant}/iot/{product}/{device}/ota/notify` | 1 | 接收OTA升级通知 |
| `{tenant}/iot/{product}/{device}/command/+` | 1 | 接收远程指令（重启/维护/锁定） |
...
```
Publish: {tenant}/iot/{product}/{device}/heartbeat  QoS 1
```
...
```
Publish: {tenant}/iot/{product}/{device}/property/post  QoS 1
```
...
订单状态变更：
```
Publish: {tenant}/iot/{product}/{device}/event/post  QoS 1
```
```json
{
...
设备收到 Broker 推送：
```
Receive: {tenant}/iot/{product}/{device}/ota/notify  QoS 1
```
```json
{
  "version":     "v2.0.0",
...
```
Publish: {tenant}/iot/{product}/{device}/ota/progress  QoS 1
```
